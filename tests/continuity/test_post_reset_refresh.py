...
def test_post_reset_refresh_keeps_knowledge_warnings_non_blocking(tmp_path):
    from hermes_continuity.refresh import run_post_reset_continuity_refresh
    ...
    with patch("hermes_continuity.refresh.generate_checkpoint", return_value={"status": "PASS", "checkpoint_id": "ckpt_1"}), patch(
        ...
        "hermes_continuity.refresh.rehydrate_latest_checkpoint", return_value={"status": "PASS"}
    ), patch(
        "hermes_continuity.refresh.verify_single_machine_readiness", return_value={"status": "PASS", "payload": {"status": "PASS"}}
    ), patch(
        "hermes_continuity.refresh.refresh_continuity_knowledge_plane",
        return_value={"health": {"status": "WARN", "operator_summary": "Knowledge Plane is usable with warnings."}},
    ):
        result = run_post_reset_continuity_refresh(
            session_key="agent:main:telegram:dm:123",
            session_id="sess_new",
    ...
    assert result["status"] == "PASS"
    assert result["informational_warning_steps"] == ["knowledge-health"]
